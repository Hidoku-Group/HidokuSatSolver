...
do
	...
	let m=$m-1
	echo "lines: $m"
	tail -n $m $filename > tmp.dat

	head -n 1 $filename | awk "{ print $sep$cutcol }" > tmpHead$col.dat
	cat tmp.dat | awk "{ print $sep$cutcol }" | sort -g > tmp$col.dat
done
...
echo "set terminal png " > tmp.plt #postscript eps color enhanced" > tmp.plt
#echo "set terminal postscript eps" > tmp.plt
echo "set key left top" >> tmp.plt
echo "set output \"$outfile.eps\"" >> tmp.plt
echo "set xlabel \"solved hidokus\"" >> tmp.plt
